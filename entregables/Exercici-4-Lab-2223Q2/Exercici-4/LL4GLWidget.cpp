...
{
  glm::mat4 TG;
  TG = glm::translate(glm::mat4(1.f), glm::vec3(6.5,0,5));
...
void LL4GLWidget::modelTransformFantasma (float posX)
{
  glm::mat4 TG;
...
void LL4GLWidget::modelTransformDiscoBall ()
{
  glm::mat4 TG;
...
void LL4GLWidget::calculaCapsaModelDiscoBall ()
{
  // Càlcul capsa contenidora i valors transformacions inicials
  float minx, miny, minz, maxx, maxy, maxz;
  minx = maxx = discoBall.vertices()[0];
  miny = maxy = discoBall.vertices()[1];
  minz = maxz = discoBall.vertices()[2];
  for (unsigned int i = 3; i < discoBall.vertices().size(); i+=3)
  {
    if (discoBall.vertices()[i+0] < minx)
      minx = discoBall.vertices()[i+0];
    if (discoBall.vertices()[i+0] > maxx)
      maxx = discoBall.vertices()[i+0];
    if (discoBall.vertices()[i+1] < miny)
      miny = discoBall.vertices()[i+1];
    if (discoBall.vertices()[i+1] > maxy)
      maxy = discoBall.vertices()[i+1];
    if (discoBall.vertices()[i+2] < minz)
      minz = discoBall.vertices()[i+2];
    if (discoBall.vertices()[i+2] > maxz)
      maxz = discoBall.vertices()[i+2];
  }
  escalaDiscoBall = 1.5/(maxy-miny);
  
  centreBaseDiscoBall[0] = (minx+maxx)/2.0; 
  centreBaseDiscoBall[1] = miny; 
  centreBaseDiscoBall[2] = (minz+maxz)/2.0;
}

void LL4GLWidget::creaBuffersDiscoBall ()
{
  // Carreguem el model de l'OBJ - Atenció! Abans de crear els buffers!
  discoBall.load("./models/DiscoBall.obj");

  // Calculem la capsa contenidora del model
  calculaCapsaModelDiscoBall ();
  
  // Creació del Vertex Array Object de la bola disco
  glGenVertexArrays(1, &VAO_DiscoBall);
  glBindVertexArray(VAO_DiscoBall);

  // Creació dels buffers del model discoBall
  GLuint VBO_DiscoBall[6];
  // Buffer de posicions
  glGenBuffers(6, VBO_DiscoBall);
  glBindBuffer(GL_ARRAY_BUFFER, VBO_DiscoBall[0]);
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*discoBall.faces().size()*3*3, discoBall.VBO_vertices(), GL_STATIC_DRAW);
  ...
  // Buffer de normals
  glBindBuffer(GL_ARRAY_BUFFER, VBO_DiscoBall[1]);
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*discoBall.faces().size()*3*3, discoBall.VBO_normals(), GL_STATIC_DRAW);
  ...
  // En lloc del color, ara passem tots els paràmetres dels materials
  // Buffer de component ambient
  glBindBuffer(GL_ARRAY_BUFFER, VBO_DiscoBall[2]);
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*discoBall.faces().size()*3*3, discoBall.VBO_matamb(), GL_STATIC_DRAW);

  glVertexAttribPointer(matambLoc, 3, GL_FLOAT, GL_FALSE, 0, 0);
  glEnableVertexAttribArray(matambLoc);

  // Buffer de component difusa
  glBindBuffer(GL_ARRAY_BUFFER, VBO_DiscoBall[3]);
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*discoBall.faces().size()*3*3, discoBall.VBO_matdiff(), GL_STATIC_DRAW);

  glVertexAttribPointer(matdiffLoc, 3, GL_FLOAT, GL_FALSE, 0, 0);
  glEnableVertexAttribArray(matdiffLoc);

  // Buffer de component especular
  glBindBuffer(GL_ARRAY_BUFFER, VBO_DiscoBall[4]);
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*discoBall.faces().size()*3*3, discoBall.VBO_matspec(), GL_STATIC_DRAW);

  glVertexAttribPointer(matspecLoc, 3, GL_FLOAT, GL_FALSE, 0, 0);
  glEnableVertexAttribArray(matspecLoc);

  // Buffer de component shininness
  glBindBuffer(GL_ARRAY_BUFFER, VBO_DiscoBall[5]);
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*discoBall.faces().size()*3, discoBall.VBO_matshin(), GL_STATIC_DRAW);
